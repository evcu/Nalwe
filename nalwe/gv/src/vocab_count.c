...
//code is modified by evcu. Additional features added
//-stopwords: Give the path of ine separeted list of words to be removed.
//-Single charachter vocablary caused by tokenization is removed. 
...
char stopword_file[100]; //Line separeted list of stop words
...
int hashdelete(HASHREC **ht, char *w) {
    HASHREC *htmp, *hprv;
    unsigned int hval = HASHFN(w, TSIZE, SEED);    
    for(hprv = NULL, htmp = ht[hval]; htmp != NULL && scmp(htmp->word, w) != 0; hprv = htmp, htmp = htmp->next);
    if(htmp != NULL) {
...
void removeStopWords(HASHREC **ht){
    FILE *fin = fopen(stopword_file, "r");
    int d_count=0,sw_count=0;
    char str[MAX_STRING_LENGTH];
    if(fin == NULL) {fprintf(stderr,"Unable to open file %s.\n",stopword_file); exit(1);}
    while(fscanf(fin, "%s\n", str) != EOF){
        sw_count++;
        if(hashdelete(ht,str)){
            d_count++;
            if(verbose>1) fprintf(stderr,"The stop-word \"%s\" is deleted\n",str);
        }
    }
    if(verbose>0) fprintf(stderr,"%d out of %d stop-words are founded and removed\n",d_count,sw_count);
    fclose(fin);
}
int get_counts() {
...
int main(int argc, char **argv) {
    ...
    if (argc == 1) {
        ...
        printf("\t\t To remove the set it to 1 \n");
        printf("\t-stopwords <int>\n");
    ...
    if ((i = find_arg((char *)"-stopwords", argc, argv)) > 0) strcpy(stopword_file, argv[i + 1]);
